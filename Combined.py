...
class AdvancedPanoramaStitcher:
    ...
    def apply_exposure_compensation(self, images):
        """Apply exposure compensation to normalize brightness across frames"""
        self.update_status("Applying exposure compensation", 0)
        ...
        for i, frame in enumerate(images):
            ...
            if i % 10 == 0:
                self.update_status("Calculating frame brightness", int((i + 1) / len(images) * 100))
        ...
        for i, frame in enumerate(images):
            ...
            # Limit the adjustment range to avoid over-correction
    ...
    def find_matching_features(self, img1, img2):
        ...
        H, mask = cv2.findHomography(dst_pts, src_pts, cv2.RANSAC, 10.0, maxIters=2000)
    ...
    def build_image_graph(self, image_paths):
        ...
        # Apply exposure compensation for more consistent brightness
        images = self.apply_exposure_compensation(images)
        ...
        keyframe_interval = 4  # Use every 20th frame as a keyframe
    ...
    def create_complete_panorama(self, image_paths):
        ...
        # Enhance colors
        #final_panorama = self.enhance_colors(sharpened_panorama)
        final_panorama = sharpened_panorama  # Skip color enhancement
